...
//        gimbal_mode_set->gimbal_pitch_motor.gimbal_motor_mode = GIMB AL_MOTOR_GYRO;
			  gimbal_mode_set->gimbal_pitch_motor.gimbal_motor_mode = GIMBAL_MOTOR_ENCONDE;
    }
...
static void gimbal_behavour_set(gimbal_control_t *gimbal_mode_set)
{
    ...
    if (gimbal_behaviour == GIMBAL_INIT)
    {
       ...
       if(gimbal_mode_set->gimbal_rc_ctrl->key.v & KEY_PRESSED_OFFSET_C)
			...
			if(gimbal_mode_set->gimbal_rc_ctrl->key.v & KEY_PRESSED_OFFSET_Q)	 
			{
				fast_shoot=1;
			}
			else
			...
			{
				while(gimbal_mode_set->gimbal_rc_ctrl->key.v & KEY_PRESSED_OFFSET_V);
				xtl_change=-xtl_change;
			}	
			
			if(gimbal_mode_set->gimbal_rc_ctrl->key.v & KEY_PRESSED_OFFSET_G)	 
			{
				while(gimbal_mode_set->gimbal_rc_ctrl->key.v & KEY_PRESSED_OFFSET_G);
				faster++;
			}				
		 if(gimbal_mode_set->gimbal_rc_ctrl->key.v & KEY_PRESSED_OFFSET_B)	 
			{
				while(gimbal_mode_set->gimbal_rc_ctrl->key.v & KEY_PRESSED_OFFSET_B);
				faster--;
			}		
...
fp32 autoshoot_yaw_data_solve = 18000.0f;
fp32 autoshoot_pitch_data_slove = 25000.0f;

fp32 autoshoot_pitch  = 0;
fp32 autoshoot_yaw		= 0; 
...
static void gimbal_absolute_angle_control(fp32 *yaw, fp32 *pitch, gimbal_control_t *gimbal_control_set)
{
    if (yaw == NULL || pitch == NULL || gimbal_control_set == NULL)
    {
	 ...
	 fp32 last_autoshoot_yaw 		= 0;
	 fp32 last_autoshoot_pitch  = 0;
   ...
   if(autoaim_mode_flag == 0)  //正常操作手模式
      ...
      rc_deadband_limit(gimbal_control_set->gimbal_rc_ctrl->rc.ch[PITCH_CHANNEL], pitch_channel, RC_DEADBAND);
			...
			//遥控器控制  +  鼠标控制   注意正负号
      *yaw = (yaw_channel * YAW_RC_SEN - gimbal_control_set->gimbal_rc_ctrl->mouse.x * YAW_MOUSE_SEN)* 1.5f;
//			*yaw = (yaw_channel * YAW_RC_SEN - gimbal_control_set->gimbal_rc_ctrl->mouse.x * YAW_MOUSE_SEN)+1.71f;
//			 *pitch = (pitch_channel * PITCH_RC_SEN -  gimbal_control_set->gimbal_rc_ctrl->mouse.y * PITCH_MOUSE_SEN)*2.0f;
			 *pitch = (pitch_channel * PITCH_RC_SEN +  gimbal_control_set->gimbal_rc_ctrl->mouse.y * PITCH_MOUSE_SEN)*2.0f;		
		}
		else if(autoaim_mode_flag == 1)  //自瞄
		{
//			*yaw =(visionDataGet.yaw_angle.yaw_predict/autoshoot_yaw_data_solve*PI/180.f)-0.5f*( gimbal_control_set->gimbal_rc_ctrl->mouse.x * YAW_MOUSE_SEN);	//12000
//			*pitch=(visionDataGet.pitch_angle.pitch_predict/autoshoot_pitch_data_slove*PI/180.f)+0.5f* (gimbal_control_set->gimbal_rc_ctrl->mouse.y * PITCH_MOUSE_SEN);	//12000
			gimbal_control_set->gimbal_yaw_motor.absolute_angle_set 		= filter_x*( (visionDataGet.yaw_angle.yaw_predict * PI/180.f/100.f)) + (1.0f - filter_x) * last_autoshoot_yaw;
			last_autoshoot_yaw = gimbal_control_set->gimbal_yaw_motor.absolute_angle_set;
			gimbal_control_set->gimbal_pitch_motor.relative_angle_set   = filter_x*( (visionDataGet.pitch_angle.pitch_predict * PI/180.f/100.f)) + (1.0f -filter_x) * last_autoshoot_pitch;
			last_autoshoot_pitch = gimbal_control_set->gimbal_pitch_motor.relative_angle_set;
			autoshoot_yaw 		=  (visionDataGet.yaw_angle.yaw_predict /100.f);
			autoshoot_pitch   = -(visionDataGet.pitch_angle.pitch_predict /100.f);
		}

    if(!autoaim_mode_flag)
    {
...
static void gimbal_xtl_control(fp32 *yaw, fp32 *pitch, gimbal_control_t *gimbal_control_set)
{
    ...
    *yaw = yaw_channel * YAW_RC_SEN - gimbal_control_set->gimbal_rc_ctrl->mouse.x * YAW_MOUSE_SEN;
	  *pitch = pitch_channel * PITCH_RC_SEN + gimbal_control_set->gimbal_rc_ctrl->mouse.y * PITCH_MOUSE_SEN;
		...
		{
//			*yaw =(visionDataGet.yaw_angle.yaw_predict/autoshoot_yaw_data_solve*PI/180)-0.5f*( gimbal_control_set->gimbal_rc_ctrl->mouse.x * YAW_MOUSE_SEN);	//12000
//			*pitch=-(visionDataGet.pitch_angle.pitch_predict/autoshoot_pitch_data_slove*PI/180)+0.5f* (gimbal_control_set->gimbal_rc_ctrl->mouse.y * PITCH_MOUSE_SEN);	//12000
