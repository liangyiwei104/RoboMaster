/**
  ...
  *                    ??????/''\ ??????
  *
  *             ???flash?,????????? name[3] ? ????? cali_flag
  *             ??head_cali?????,????12???flash,????0x080A0000??
  *             0x080A0000-0x080A0007: head_cali??
  *             0x080A0008: ??name[0]
  *             0x080A0009: ??name[1]
  *             0x080A000A: ??name[2]
  *             0x080A000B: ????? cali_flag,???????0x55,???head_cali?????
  *             ?????
  *             1.??????calibrate_task.h?cali_id_e, ?
  *             typedef enum
  *             {
  *                 ...
  *                 //add more...
  ...
  *             3.? "FLASH_WRITE_BUF_LENGHT",??"sizeof(xxx_cali_t)", ??????
  *             bool_t cali_xxx_hook(uint32_t *cali, bool_t cmd), ?????? "cali_name[CALI_LIST_LENGHT][3]"
  *             ????? xxx_cali_t xxx_cail, ???????cali_sensor_buf[CALI_LIST_LENGHT]
  *             ?cali_sensor_size[CALI_LIST_LENGHT]??????, ???cali_hook_fun[CALI_LIST_LENGHT]????
  *
  ==============================================================================
  @endverbatim
  ****************************(C) COPYRIGHT 2019 DJI****************************
...
/**
  * @brief          "head"????
  * @param[in][out] cali:????head??,?cmd?CALI_FUNC_CMD_INIT, ?????,CALI_FUNC_CMD_ON,?????
  * @param[in]      cmd: 
                    CALI_FUNC_CMD_INIT: ??????????????
                    CALI_FUNC_CMD_ON: ??????
  * @retval         0:????????
...
/**
  * @brief          ???????
  * @param[in][out] cali:?????????,?cmd?CALI_FUNC_CMD_INIT, ?????,CALI_FUNC_CMD_ON,?????
  * @param[in]      cmd: 
                    CALI_FUNC_CMD_INIT: ??????????????
                    CALI_FUNC_CMD_ON: ??????
  * @retval         0:????????
